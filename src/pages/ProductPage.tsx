        ...
        <div className="container mx-auto px-4 lg:px-8 py-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="aspect-square bg-card overflow-hidden border border-border"
            >
            ...
            >
              ...
              <h1 className="heading-display text-3xl md:text-5xl font-bold mt-1 mb-4">{product.name}</h1>

              <div className="flex items-center gap-3 mb-6">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={`w-4 h-4 ${i < Math.floor(product.rating) ? 'fill-neon text-neon' : 'text-muted'}`} />
                  ))}
                </div>
              ...
              <div className="flex items-center gap-3 mb-8">
                ...
                {product.originalPrice && (
                  <span className="bg-hot text-foreground px-2 py-1 text-xs font-body font-bold tracking-wider uppercase">
              ...
              {/* Size */}
              <div className="mb-6">
                <h3 className="font-heading font-bold uppercase tracking-wider text-sm mb-3">Select Size</h3>
                <div className="flex flex-wrap gap-2">
                  {product.sizes.map(size => (
                    <button
                      key={size}
                      onClick={() => setSelectedSize(size)}
                      className={`w-12 h-12 border font-body text-sm font-semibold transition-all ${
                        selectedSize === size
                          ? 'border-neon bg-neon text-accent-foreground glow-neon'
              ...
              {/* Color */}
              <div className="mb-8">
                <h3 className="font-heading font-bold uppercase tracking-wider text-sm mb-3">Color</h3>
                <div className="flex flex-wrap gap-2">
                  {product.colors.map(color => (
                    <button
                      key={color}
                      onClick={() => setSelectedColor(color)}
                      className={`px-4 py-2 border font-body text-sm font-medium transition-all ${
                        selectedColor === color
                          ? 'border-neon bg-neon text-accent-foreground'
                          : 'border-border hover:border-neon/50'
                      }`}
                    >
                      {color}
              ...
              <div className="flex items-center gap-4 mb-8">
                ...
                <button
                  ...
                  className="flex-1 h-12 bg-neon text-accent-foreground font-body text-sm font-bold tracking-wider uppercase hover:bg-neon-glow transition-all duration-300 glow-neon"
                >
                  Add to Cart
                </button>
                <button
                  onClick={() => toggleWishlist(product.id)}
                  className={`w-12 h-12 border flex items-center justify-center transition-all ${wishlisted ? 'border-neon bg-neon/10' : 'border-border hover:border-neon/50'}`}
                >
                  <Heart className={`w-5 h-5 ${wishlisted ? 'fill-neon text-neon' : ''}`} />
                </button>
              </div>
          ...
          {related.length > 0 && (
            <section className="mt-20 pt-10 border-t border-border">
              <h2 className="heading-display text-2xl md:text-4xl font-bold mb-10">You May Also Like</h2>
