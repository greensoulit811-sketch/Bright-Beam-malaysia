          ...
          <div className="flex items-center h-20">
            ...
            <div className="flex-1 flex justify-center h-full">
              <div className="flex items-center gap-6 xl:gap-8 h-full">
                {navLinks.map((link) => (
                  <div key={link.title} className="relative h-20 flex items-center group cursor-pointer" onMouseEnter={() => setHoveredLink(link.title)} onMouseLeave={() => setHoveredLink(null)}>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            >
              {/* Drawer Top Row: Close Button + Logo/Search? */}
              ...
              {/* Tabs Section (As requested in image) */}
              <div className="flex border-b border-gray-100 bg-gray-50/50">
                 <button 
                  onClick={() => setMobileTab('menu')}
                  className={`flex-1 py-4 text-center font-bold text-[15px] transition-all relative ${mobileTab === 'menu' ? 'text-blue-600 bg-white' : 'text-gray-500'}`}
                 >
                   Menu
                   {mobileTab === 'menu' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600" />}
                 </button>
                 <button 
                  onClick={() => setMobileTab('categories')}
                  className={`flex-1 py-4 text-center font-bold text-[15px] transition-all relative ${mobileTab === 'categories' ? 'text-blue-600 bg-white' : 'text-gray-500'}`}
                 >
                   Categories
                   {mobileTab === 'categories' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600" />}
                 </button>
              </div>
                  ...
                  {mobileTab === 'menu' ? (
                       ...
                       {navLinks.map((link) => (
                         <div key={link.title} className="flex flex-col">
                            <div className="flex items-center justify-between p-4 bg-white hover:bg-gray-50 transition-colors">
                               {link.dropdown ? (
                                 <span className="text-[14px] font-bold text-[#0A2342] flex-1">{link.title}</span>
                               ) : (
                                 <Link to="/shop" onClick={() => setMobileOpen(false)} className="text-[14px] font-bold text-[#0A2342] flex-1">{link.title}</Link>
                               )}
                               {link.dropdown && (
                                 <button onClick={() => toggleExpanded(link.title)} className="p-1 px-3 border border-gray-100 rounded text-gray-400 hover:text-blue-600 transition-colors">
                                   {expandedLinks.includes(link.title) ? <Minus className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                                 </button>
                               )}
                            </div>
                            {link.dropdown && expandedLinks.includes(link.title) && (
                              <div className="bg-gray-50 divide-y divide-gray-100">
                                {link.items?.map(item => (
                                  <Link key={item.id} to={`/shop?category=${item.slug}`} className="block p-4 pl-8 text-[13px] font-semibold text-gray-500 hover:text-blue-600" onClick={() => setMobileOpen(false)}>{item.name}</Link>
                                ))}
                              </div>
                            )}
                         </div>
                    ...
                    <motion.div key="cat" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="divide-y divide-gray-50">
                       {parentCategories.map(cat => (
                         <Link key={cat.id} to={`/shop?category=${cat.slug}`} className="flex items-center justify-between p-4 text-[14px] font-bold text-[#0A2342] bg-white hover:bg-gray-50 transition-colors" onClick={() => setMobileOpen(false)}>
                           {cat.name} <ChevronDown className="w-4 h-4 -rotate-90 text-gray-300" />
                         </Link>
                       ))}
                    </motion.div>
                  )}
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100 space-y-4">
                 <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                       <span className="text-[10px] font-bold text-gray-400 uppercase leading-none">Support</span>
                       <a href={`tel:${hotline}`} className="text-sm font-black text-[#0A2342]">{hotline}</a>
                    </div>
                 </div>
                 <div className="flex items-center justify-between px-1">
                    <div className="flex items-center gap-2 text-gray-400"><Globe className="w-4 h-4" /> <span className="text-[10px] font-bold uppercase">Language</span></div>
                    <LanguageSwitcher />
                 </div>
              </div>
            </motion.div>
